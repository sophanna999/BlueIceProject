    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-sm" id="ListWarehouse">
            <thead>
              <tr>
      ...
      <form id="FormPrimary" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            ...
            <input name="warehouse_name" class="form-control" placeholder="Warehouse Name" value="" type="text">
          </div>
        </div>
        <!-- End Body -->
    ...
    <div class="modal-content">
      ...
      <form id="FormWarning" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">คลังสินค้า</label>
            <input name="warehouse_name" id="warehouse_name" class="form-control" placeholder="Warehouse Name" type="text">
          </div>
        </div>
        <!-- End Body -->
        <div class="modal-footer">
...
<!-- DataTables -->
<script>
var dataTableList = $('#ListWarehouse').dataTable({
  "focusInvalid": false,
  "processing": true,
  "serverSide": true,
  "ajax": {
    "url": url+"/Setting/listWarehouse",
    "data": function ( d ) {
    }
  },
  "columns": [
...
$('body').on('click', '.btn-edit', function(){
  ...
  $.ajax({
    method : "GET",
    url : url+"/Setting/Warehouse/"+id,
    dataType : 'json'
  }).done(function(rec){
    $('#editid').val(rec.id);
    $('#warehouse_name').val(rec.warehouse_name);
...
$('body').on('click', '.btn-delete',function(){
  ...
  swal({
    ...
    if (result.value==true) {
      $.ajax({
        method : "GET",
        url : url+"/Setting/Warehouse/delete/"+id,
        dataType : 'json'
      }).done(function(rec){
        if(rec.type=='success'){
          swal({
...
$( "#FormWarning" ).validate({
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/Warehouse/update",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
