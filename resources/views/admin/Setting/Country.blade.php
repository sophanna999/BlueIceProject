    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-sm" id="ListCountry">
            <thead>
              <tr>
                <th class="text-center">ลำดับ</th>
                <th class="text-center">รหัสประเทศ</th>
                <th class="text-center">ชื่อประเทศ(ไทย)</th>
                <th class="text-center">ชื่อประเทศ(อังกฤษ)</th>
      ...
      <form id="FormPrimary" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">ชื่อประเทศ(ไทย)</label>
            <input name="name_th" class="form-control" placeholder="ชื่อประเทศ(ไทย)" value="" type="text">
          </div>
          <div class="form-group col-md-12">
            <label class="control-label">ชื่อประเทศ(อังกฤษ)</label>
            <input name="name_en" class="form-control" placeholder="ชื่อประเทศ(อังกฤษ)" value="" type="text">
          </div>
          <div class="form-group col-md-12">
    ...
    <div class="modal-content">
      ...
      <form id="FormWarning" class="form-horizontal">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">ชื่อประเทศ(ไทย)</label>
            <input name="name_th" id="name_th" class="form-control" placeholder="ชื่อประเทศ(ไทย)" value="" type="text">
          </div>
          <div class="form-group col-md-12">
            <label class="control-label">ชื่อประเทศ(อังกฤษ)</label>
            <input name="name_en" id="name_en" class="form-control" placeholder="ชื่อประเทศ(อังกฤษ)" value="" type="text">
          </div>
...
<!-- DataTables -->
<script>
var dataTableList = $('#ListCountry').dataTable({
    "focusInvalid": false,
    "processing": true,
    "serverSide": true,
    "ajax": {
      "url": url+"/Setting/Country/listCountry",
      "data": function ( d ) {
      }
    },
    "columns": [
...
$('body').on('click', '.btn-edit', function(){
  ...
  $.ajax({
    method : "GET",
    url : url+"/Setting/Country/"+id,
    dataType : 'json'
  }).done(function(rec){
    $('#name_th').val(rec.name_th);
    $('#name_en').val(rec.name_en);
    $('#editid').val(rec.country_id);
...
$('body').on('click', '.btn-delete',function(){
  ...
  swal({
    ...
    if (result.value==true) {
      $.ajax({
        method : "GET",
        url : url+"/Setting/Country/delete/"+id,
        dataType : 'json'
      }).done(function(rec){
        if(rec.type=='success'){
          swal({
...
$( "#FormWarning" ).validate({
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/Country/update",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
