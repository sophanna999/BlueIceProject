    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-sm" id="ListAccountTypeMaster">
            <thead>
              <tr>
                <th class="text-center">รหัสหมวดบัญชี</th>
      ...
      <form id="FormPrimary" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            ...
            <input name="accountType_name" class="form-control" placeholder="Account Type Master Name" value="" type="text">
          </div>
        </div>
        <!-- End Body -->
    ...
    <div class="modal-content">
      ...
      <form id="FormWarning" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">หมวดบัญชี</label>
            <input name="accountType_name" id="accountType_name" class="form-control" placeholder="Account Type Master Name" type="text">
          </div>
        </div>
        <!-- End Body -->
        <div class="modal-footer">
...
var dataTableList = $('#ListAccountTypeMaster').dataTable({
  ...
  "serverSide": true,
  "ajax": {
    "url": url+"/Setting/listAccountTypeMaster",
    "data": function ( d ) {
    }
  },
  "columns": [
...
$('body').on('click', '.btn-edit', function(){
  ...
  $.ajax({
    method : "GET",
    url : url+"/Setting/AccountTypeMaster/"+id,
    dataType : 'json'
  }).done(function(rec){
    $('#editid').val(rec.id);
    $('#accountType_name').val(rec.accountType_name);
...
$('body').on('click', '.btn-delete',function(){
  ...
  swal({
    ...
    if (result.value==true) {
      $.ajax({
        method : "GET",
        url : url+"/Setting/AccountTypeMaster/delete/"+id,
        dataType : 'json'
      }).done(function(rec){
        if(rec.type=='success'){
          swal({
...
$( "#FormPrimary" ).validate({
  rules: {
    accountType_name: "required",
  },
  messages: {
    accountType_name: "กรุณาระบุ",
  },
  errorElement: "span",
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/AccountTypeMaster",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
...
$( "#FormWarning" ).validate({
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/AccountTypeMaster/update",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
