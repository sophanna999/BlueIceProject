    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-sm" id="ListUnitType">
            <thead>
              <tr>
                <th class="text-center">รหัสประเภทหน่วยนับ</th>
                <th class="text-center">ประเภทหน่วยนับ</th>
      ...
      <form id="FormPrimary" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            ...
            <input name="unitType_name" class="form-control" placeholder="Unit Type Name" value="" type="text">
          </div>
        </div>
        <!-- End Body -->
    ...
    <div class="modal-content">
      ...
      <form id="FormWarning" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">ประเภทหน่วยนับ</label>
            <input name="unitType_name" id="unitType_name" class="form-control" placeholder="Unit Type Name" type="text">
          </div>
        </div>
        <!-- End Body -->
        <div class="modal-footer">
...
<!-- DataTables -->
<script>
var dataTableList = $('#ListUnitType').dataTable({
  "focusInvalid": false,
  "processing": true,
  "serverSide": true,
  "ajax": {
    "url": url+"/Setting/listUnitType",
    "data": function ( d ) {
    }
  },
  "columns": [
    { "data": "id" , "name":"id" , "className": "text-center" },
    { "data": "unitType_name" , "name":"unitType_name" },
...
$('body').on('click', '.btn-edit', function(){
  ...
  $.ajax({
    method : "GET",
    url : url+"/Setting/UnitType/"+id,
    dataType : 'json'
  }).done(function(rec){
    $('#editid').val(rec.id);
    $('#unitType_name').val(rec.unitType_name);
...
$('body').on('click', '.btn-delete',function(){
  ...
  swal({
    ...
    if (result.value==true) {
      $.ajax({
        method : "GET",
        url : url+"/Setting/UnitType/delete/"+id,
        dataType : 'json'
      }).done(function(rec){
        if(rec.type=='success'){
          swal({
...
$( "#FormPrimary" ).validate({
  rules: {
    unitType_name: "required",
  },
  messages: {
    unitType_name: "กรุณาระบุ",
  },
  errorElement: "span",
...
$( "#FormWarning" ).validate({
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/UnitType/update",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
