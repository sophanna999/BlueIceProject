    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-hover table-sm" id="ListPaymentType">
            <thead>
              <tr>
                <th class="text-center">รหัสประเภทการจ่ายเงิน </th>
      ...
      <form id="FormPrimary" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            ...
            <input name="paymentType_name" class="form-control" placeholder="Payment Type Name" value="" type="text">
          </div>
        </div>
        <!-- End Body -->
    ...
    <div class="modal-content">
      ...
      <form id="FormWarning" method="post" class="form-horizontal" action="">
        ...
        <div class="modal-body">
          <div class="form-group col-md-12">
            <label class="control-label">ประเภทการจ่ายเงิน</label>
            <input name="paymentType_name" id="paymentType_name" class="form-control" placeholder="Payment Type Name" type="text">
          </div>
        </div>
        <!-- End Body -->
        <div class="modal-footer">
...
<!-- DataTables -->
<script>
var dataTableList = $('#ListPaymentType').dataTable({
  "focusInvalid": false,
  "processing": true,
  "serverSide": true,
  "ajax": {
    "url": url+"/Setting/listPaymentType",
    "data": function ( d ) {
    }
  },
  "columns": [
...
$('body').on('click', '.btn-edit', function(){
  ...
  $.ajax({
    method : "GET",
    url : url+"/Setting/PaymentType/"+id,
    dataType : 'json'
  }).done(function(rec){
    $('#editid').val(rec.id);
    $('#paymentType_name').val(rec.paymentType_name);
...
$('body').on('click', '.btn-delete',function(){
  ...
  swal({
    ...
    if (result.value==true) {
      $.ajax({
        method : "GET",
        url : url+"/Setting/PaymentType/delete/"+id,
        dataType : 'json'
      }).done(function(rec){
        if(rec.type=='success'){
          swal({
...
$( "#FormPrimary" ).validate({
  rules: {
    paymentType_name: "required",
  },
  messages: {
    paymentType_name: "กรุณาระบุ",
  },
  errorElement: "span",
...
$( "#FormWarning" ).validate({
  ...
  submitHandler: function(form){
    ...
    $.ajax({
      method : "POST",
      url : url+"/Setting/PaymentType/update",
      dataType : 'json',
      data : $(form).serialize()
    }).done(function(rec){
      if(rec.type == 'success'){
