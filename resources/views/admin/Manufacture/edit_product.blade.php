    ...
    <div class="card card-accent-primary">
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <div class="input-group">
                            <input type="text" id="rp_no" name="rp_no" class="form-control" value="{{$BomOrderDetail->rp_no}}" readonly>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <div class="input-group">
                        <input type="text" id="rp_date" name="rp_date" class="form-control datecalendar" value="{{$BomOrderDetail->bom_date}}" placeholder="dd/mm/yyyy">
                            <span class="input-group-btn">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <div class="row">
                            <div class="input-group col-md-8">
                                <textarea name="rp_notation" class="form-control">{{$BomOrderDetail->bom_comment}}</textarea>
                            </div>
                        </div>
                    </div>
                </div><!-- end row -->
        ...
        <table class="table table-bordered table-striped table-sm">
            ...
            <tbody>
                ...
                <tr>
                    <td>1</td>
                    <td>{{$BomOrderDetail->task_no}}</td>
                    <td>{{$BomOrderDetail->pro_id}}</td>
                    <td>{{$BomOrderDetail->ProName}}</td>
                    <td><input style="text-align: right;" class="form-control" tyle="text" value="{{$BomOrderDetail->amount}}" readonly></td>
                    <td>
                        <select class="form-control" name="branch_reciept">
                            <option value="">กรุณาเลือก</option>
                            @foreach($branch as $v)
                            <option value="{{$v->BraID}}" {{($v->BraID == $BomOrderDetail->BraID)?'selected':''}}>{{$v->BraName}}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="form-control" name="warehouse_id">
                            ...
                            <option value="{{$v->id}}" {{($v->id == $BomOrderDetail->warehouse_id)?'selected':''}}>{{$v->warehouse_name}}</option>
                            @endforeach
                        </select>
                    </td>
                    <td><input style="text-align: right;" class="form-control" name="pass_amount" value="{{$BomOrderDetail->pass_amount}}"/></td>
                    <td><input style="text-align: right;" class="form-control" name="corrupt_amount" value="{{$BomOrderDetail->corrupt_amount}}"/></td>
                    <td style="text-align:center;vertical-align:middle;">
                        <input class="ProID" type="hidden" id="ProID" name="ProID" value="{{$BomOrderDetail->ProID}}">
                        <button type="button" name="material_restore" class="btn btn-sm btn-outline-warning material_restore" data-id="">รายละเอียด</button>
                    </td>
                    <td style="text-align: center; vertical-align: middle;"><input type="checkbox" name="status" value="T"></td>
                </tr>
            </tbody>
        </table>
        <div class="card-footer" style="text-align: center;">
            <button type="submit" class="btn btn-sm btn-primary save"><i class="fa fa-dot-circle-o"></i> บันทึก</button>
            <button type="reset" class="btn btn-sm btn-danger"><i class="fa fa-ban"></i> ยกเลิก</button>
    ...
    $.ajax({
        ...
        $.each(rec,function(k,v) {
            ...
            if($.inArray(v.MatCode,arr)== -1) {
                ...
                <td><input type="text" name="restore['+k+']" class="form-control"></td>\n\
                <td>'+v.name_th+'</td>\n\
                </tr>';
            }
        });
...
$( "#FormPrimary" ).validate({
    ...
    submitHandler: function(form){
        var btn = $(form).find('[type="submit"]');
        if($('body').find('input:checkbox[name*="status"]').prop('checked')==true) {
            $.ajax({
                method : "POST",
