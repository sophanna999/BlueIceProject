                    ...
                    <div class="row">
                        <div class="form-group col-sm-12">
                            <table class="table">
                                ...
                                <tr>
                                    <td style="width:5%;"> <input type="checkbox" class="ProID" id="ProID[{{$k}}]" name="ProID[{{$k}}]" value="{{$data->ProID}}"> </td>
...
// $('body').on('click', '.btn-addpro', function () {
//     var selectpro = '';
...
//     <div class="form-group col-sm-6">\n\
//     <label for="ProID">สินค้า</label>\n\
//     <select class="form-control ProID' + index + '" name="ProID[' + index + ']">\n\
//     <option value="">เลือก</option>\n\
//     ' + selectpro + '\n\
//     </select>\n\
//     </div>\n\
//     <div class="form-group col-sm-6">\n\
//     <label for="ProNumber">จำนวน</label>\n\
//     <input type="number" class="form-control ProNumber' + index + '" name="ProNumber[' + index + ']">\n\
//     </div>\n\
//     </div>\n\
//     ')
//     $('.ProID' + index + '[name="ProID[' + index + ']"]').rules("add", {
//         required: true,
//         messages: {
//             required: "กรุณาระบุ",
//         }
//     });
//     $('.ProNumber' + index + '[name="ProNumber[' + index + ']"]').rules("add", {
//         required: true,
//         number: true,
//         messages: {
//             required: "กรุณาระบุ",
...
$('body').on('click', '.btn-add', function () {
    ...
    $('body').on('click', '.btn-edit', function () {
        ...
        $.ajax({
            ...
            $.each(rec, function( key, value ) {
                console.log(value);
                $('.D_TruckProID').val(value.TruckProID);
                $('.D_TruckID').val(value.TruckID).prop('disabled',true);
                $('body').find('input:checkbox[value="'+value.ProID+'"]').closest('tr').find('.U_ProNumber').val(value.ProNumber);
                $('body').find('input:checkbox[value="'+value.ProID+'"]').prop('checked', true);
                $('.D_TruckDate').val(value.TruckDate).prop('disabled',true);
                if(value.status=='T') {
                    $('body').find('input:checkbox[value="'+value.ProID+'"]').attr('disabled', 'disabled');
                    $('body').find('input:checkbox[value="'+value.ProID+'"]').closest('tr').find('.U_ProNumber').attr('readonly', 'readonly');
                } else {
                    $('body').find('input:checkbox[value="'+value.ProID+'"]').removeAttr('disabled');
                    $('body').find('input:checkbox[value="'+value.ProID+'"]').closest('tr').find('.U_ProNumber').removeAttr('readonly');
                }
                $('.RoundAmount').val(value.RoundAmount);
                $('.driver').val(value.driver);
            });
    ...
    $("#FormUpdateTruck").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                method: "POST",
                url: url + "/Market/Pickup/update/"+id,
                dataType: 'json',
                data: $(form).serialize()
            }).done(function (rec) {
                if (rec.type == 'success') {
